...
PACKETS = HERE / "redesign_results/final_development_v6"
CONFIG_PATH = HERE / "examples/moderate_severe_variable_environment.json"
EXPLORATORY_ROOT = 35_000_000
BOOTSTRAP = 10_000
RNG_SEED = 35_000_999          # analysis-only; affects no simulation
...
def load():
    ...
    for path in sorted(PACKETS.glob("*.json")):
        ...
        if packet["identity"]["root"] != EXPLORATORY_ROOT:
            raise RuntimeError(f"foreign packet in the V6 directory: {path}")
...
def main():
    ...
    report = {"schema": "study3_final_development_v6_analysis_v1",
    ...
    path = HERE / "redesign_results/final_development_v6_analysis.json"
    ...
    for label in ("reactive_minus_deployment", "predictive_minus_deployment",
                  "predictive_minus_reactive"):
        print(f"  [{label}]")
        for field in METRICS:
