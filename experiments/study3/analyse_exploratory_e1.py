...
**No threshold is applied and no verdict is assigned.** This is exploratory
evidence. It cannot revise the held-out result or the freeze decision.
...
PACKETS = HERE / "redesign_results/exploratory_e1"
CONFIG_PATH = HERE / "examples/moderate_severe_variable_environment.json"
EXPLORATORY_ROOT = 33_000_000
BOOTSTRAP = 10_000
RNG_SEED = 33_000_999          # analysis-only; affects no simulation
...
def load():
    ...
    for path in sorted(PACKETS.glob("*.json")):
        ...
        if packet["identity"]["root"] != EXPLORATORY_ROOT:
            raise RuntimeError(f"non-exploratory packet in the E1 directory: {path}")
...
def main():
    ...
    table = {}
    for row in rows:
        table.setdefault(row["environment_seed"], {})[row["policy"]] = row
    ...
    report = {"schema": "study3_exploratory_e1_analysis_v1",
              "classification": "POST_FREEZE_EXPLORATORY",
              ...
              "root": EXPLORATORY_ROOT, "packets": len(packets),
              "paired_environments": len(keys),
              "means": {field: {policy: mean(policy, field) for policy in POLICIES}
                        for field in METRICS}}

    contrasts = {}
    for treatment, comparator, label in (("reactive", "fixed", "reactive_minus_fixed"),
                                         ("reactive", "deployment_fixed",
                                          "reactive_minus_deployment"),
                                         ("deployment_fixed", "fixed",
                                          "deployment_minus_fixed")):
        for field in METRICS:
    ...
    report["adaptation_pilot_definition"] = {"reactive": adaptation_pilot(packets, "reactive")}

    path = HERE / "redesign_results/exploratory_e1_analysis.json"
    ...
    print("\nmeans by policy")
    print(f"  {'metric':26s} {'FIXED':>12s} {'DEPLOY_FIXED':>14s} {'REACTIVE':>12s}")
    for field in METRICS:
        m = report["means"][field]
        print(f"  {field:26s} {m['fixed']:12.4f} {m['deployment_fixed']:14.4f} "
              f"{m['reactive']:12.4f}")
    print("\npaired contrasts (95% paired bootstrap, 60 environments)")
    for label in ("reactive_minus_fixed", "reactive_minus_deployment",
                  "deployment_minus_fixed"):
        print(f"  [{label}]")
        for field in METRICS:
